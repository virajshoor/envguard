...
function checkRange(value, baseType, args) {
  ...
  const rangeValidator = baseType === 'integer' ? isInteger : isNumber;

  if (args.length !== 2 || args.some((arg) => !rangeValidator(arg))) {
...
function validateKey(env, rule) {
  ...
  if (!present) {
    if (rule.defaultValue !== undefined) {
      const defaultTypeError = checkType(rule.defaultValue, rule.type);
      if (defaultTypeError) {
        return {
          key: rule.key,
          type: rule.type,
          ...
          value: undefined,
          pass: false,
          reason: `Invalid schema default: ${defaultTypeError}`
        };
      }

      if (rule.pattern && !rule.pattern.test(rule.defaultValue)) {
        return {
          key: rule.key,
          type: rule.type,
          required: rule.required,
          value: undefined,
          pass: false,
          reason: `Invalid schema default: does not match ${rule.pattern}`
        };
      }
      ...
      return {
        ...
        value: undefined,
        pass: true,
        reason: 'Default declared in schema'
      };
    }
  ...
  if (isEmpty(value)) {
    if (rule.allowEmpty) {
      return {
        ...
        value,
        pass: true,
        reason: 'Empty value allowed by schema'
      };
    }
  ...
  if (rule.deprecated) {
    return {
      ...
      pass: true,
      severity: 'warning',
      reason: rule.deprecatedReason || 'Deprecated key is still set'
    };
  }
...
function validateCondition(env, condition) {
  ...
  if (condition.type === 'require-if-missing') {
    if (Object.prototype.hasOwnProperty.call(env, condition.sourceKey) && !isEmpty(sourceValue)) {
      return null;
    }
  ...
  if (condition.type === 'forbidden-if') {
    if (sourceValue !== condition.expectedValue) {
      return null;
    }
...
function validate(env, schema, options = {}) {
  ...
  conditions.forEach((condition) => {
    ...
    if (result) {
      conditionFailures.push(result);
    }
  });

  const conditionFailureKeys = new Set(conditionFailures.map((result) => result.key));
  const results = Object.values(schema)
    .map((rule) => validateKey(env, rule))
    .filter((result) => {
      return !(conditionFailureKeys.has(result.key) && result.pass && result.reason === 'Optional key not set');
    });

  results.push(...conditionFailures);
